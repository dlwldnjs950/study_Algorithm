...
public class SWEA_5656_벽돌깨기고른인덱스저장O {
	...
	public static void main(String[] args) throws IOException {
		...
		for (int testCase = 1; testCase <= TC; testCase++) {
			...
			shotCol = new int[shotCnt];
			shotMarble(0);

			if (minBricks == Integer.MAX_VALUE)
				sb.append(0);
			else
				sb.append(minBricks);
			sb.append("\n");
	...
	private static void shotMarble(int cnt) {
		...
		if (cnt == shotCnt) {
			...
			// 최대한 많은 벽돌을 제거하는게 목표
			// 남아있는 벽돌 카운트 = maxHeight 배열 값 더해주면 될듯~!
...
//System.out.println("제거하는 열 : " + idx);
//for (int[] tmp : map) {
//	System.out.println(Arrays.toString(tmp));
//}
//
//System.out.println("최고높이 배열");
//System.out.println(Arrays.toString(maxHeight));
