...
public class Main {
	...
	static int height, width, breakLimit, result;
	static char map[][];
	...
	public static void main(String[] args) throws IOException {
		...
		breakLimit = Integer.parseInt(st.nextToken());
		
		// 공간 정보 저장
		map = new char[height][width];
		
		// 벽을 부수는 해당 횟수에 방문 여부
		isVisited = new boolean[height][width][breakLimit +1];
		...
		while (!q.isEmpty()) {
			...
			for (int direction = 0; direction < dr.length; direction++) {
				...
				// 벽이면
					...
					if(current.breakCnt == breakLimit)
						continue;
